...
POST   /notifications/devices          تسجيل جهاز لإشعارات النظام (push)
DELETE /notifications/devices/:token   إلغاء تسجيل جهاز
```
...
```
POST   /notifications/devices          { token, platform }  تسجيل جهاز
DELETE /notifications/devices/:token   إلغاء التسجيل (عند الخروج)
```
...
**`PushDevice`:** صف لكل جهاز، لا عمود على `User`، لأن للمستخدم الواحد أجهزة
متعددة (هاتف + لوح، أو إعادة تثبيت تُصدِر رمزًا جديدًا) وعمودٌ واحد كان
سيُسكِت الجهاز الأقدم صامتًا. `token` فريد، فالجهاز نفسه إذا سجّل دخول مستخدم
آخر **يُنقَل** الصف إليه بدل تكراره — لا يبقى الهاتف يستقبل مهام المستخدم
السابق. الحذف متتالٍ مع المستخدم، و`DELETE` مقيَّد بصاحب الرمز حتى لا يُلغي
مستخدم تسجيل جهاز غيره.
